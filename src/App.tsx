...
export default function App(){
  ...
  const [showLocal,setShowLocal]=useState(true); const [showNvis,setShowNvis]=useState(true); const [showHops,setShowHops]=useState(true);
  const result = useMemo(()=>calculatePropagation({bandId,radiationProfileId,condition,time,maxHops:5}),[bandId,radiationProfileId,condition,time]);
  const nvisAvailable = Boolean(result.nvisCoreZone || result.nvisFringeZone);
  const zones=[showLocal?result.localZone:undefined,showNvis?result.nvisCoreZone:undefined,showNvis?result.nvisFringeZone:undefined,result.skipZone,showHops?result.hopZones:undefined].flat().filter(Boolean) as any[];
